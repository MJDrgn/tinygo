...
func newPIO(index uint8) (pio *PIO) {
	if index < 0 || index > 1 {
		panic("invalid PIO index")
	}
...
func (pio *PIO) AddProgram(program *PIOProgram) (location uint8, err error) {
	offset := pio.findOffsetForProgram(program)
	if offset < 0 {
		return 0, errors.New("no program space")
	}
	err = pio.AddProgramAtOffset(program, uint8(offset))
	if err != nil {
		return 0, err
	}
...
func (pio *PIO) AddProgramAtOffset(program *PIOProgram, offset uint8) (err error) {
	if !pio.CanAddProgramAtOffset(program, offset) {
		return errors.New("no program space")
	}
...
// ClkDivRestart restarts a state machine clock divider with a phase of 0
...
// SetConsecutivePinDirs sets a range of pins to either 'in' or 'out'
...
// XORRegister gets the 'XOR' alias for a register
//
